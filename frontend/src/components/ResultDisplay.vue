<template>
  <div class="border border-border rounded-[12px] bg-card p-8" v-if="result">
    <h3 class="mt-0 text-[48px] font-semibold leading-normal text-primary tracking-[-0.96px] mb-8">評価結果</h3>

    <div class="mb-8">
      <h4 class="text-[24px] font-medium text-primary mb-4">文字起こし</h4>
      <div class="p-6 bg-bg-section rounded-[8px] border border-border">
        <p class="m-0 text-[24px] font-medium text-primary leading-[1.5]">{{ result.transcribed_text }}</p>
      </div>
    </div>

    <div class="mb-8">
      <h4 class="text-[24px] font-medium text-primary mb-6">発音精度</h4>
      <div class="flex flex-col items-center gap-6">
        <div class="w-[120px] h-[120px] rounded-full flex items-center justify-center text-white border-4 border-card shadow-button" :class="accuracyCircleClass">
    ...
    <div class="mb-8">
      <h4 class="text-[24px] font-medium text-primary mb-6">音素比較</h4>
      <div class="flex flex-col gap-6">
        <div class="flex flex-col gap-2">
          <label class="text-[16px] font-medium text-primary">期待される音素:</label>
          <div class="p-4 rounded-[8px] font-mono text-[16px] break-all bg-card border border-border text-primary">
            {{ result.expected_phonemes }}
          </div>
        </div>
        <div class="flex flex-col gap-2">
          <label class="text-[16px] font-medium text-primary">実際の音素:</label>
          <div class="p-4 rounded-[8px] font-mono text-[16px] break-all bg-card border border-border text-primary">
  ...
  <div v-else-if="loading" class="flex flex-col items-center justify-center p-12 gap-4">
    ...
    <p class="text-[16px] font-normal text-primary">評価中...</p>
  </div>

  <div v-else-if="error" class="p-6 bg-bg-section border border-border rounded-[8px] mt-6">
    <p class="m-0 text-[16px] font-normal text-primary">❌ {{ error }}</p>
  </div>
</template>
